...
data <- read.table(glob_metrics_file, skip = 3, header = FALSE, sep = "\t")
colnames(data) <- c("library", "genome_length", "nb_mapped_reads", "nb_mapped_nt", "p_coverage")
...
nb_aligned_reads <- round(sum(data$p_coverage) / 100 * nb_reads_clean)
...
data_2 <- merge(species_info, 
                data[, c("library", "p_coverage", "nb_mapped_reads")], 
                by = "library")
data_2 %>%
  arrange(type, desc(p_coverage)) -> data_2
...
# plot
ggplot(data_2, aes(x = type, y = p_coverage, fill = fct_inorder(species))) +
...
data_3 <- merge(species_info, 
                ...
                by = "library")
data_3 %>%
  arrange(desc(p_coverage)) -> data_3

ggplot(data_3, aes(x = "", y = fct_inorder(species), size = nb_mapped_reads, color = type)) +
  ...
  scale_colour_manual(labels = c("aceto" = "Acetobacteraceae",
                      ...
                      values = c("aceto" = "#115379FF",
                                 ...
                                 "yeast" = "#186D2EFF",
                                 "other" = "#76388AFF")) +
  scale_size_area(labels = comma, name = "number of reads") +
  labs(x = "", y = "species", 
       title = "Proportion of reads mapped on each species")
```
