...
for (file in files) {
  len_sample_name <- nchar(get_sample_name(file))
  sample_name <- if (len_sample_name <= 20) {
    get_sample_name(file)
  } else {
    substr(get_sample_name(file), len_sample_name - 19, len_sample_name)
  }
...
ggplot(data_2, aes(x = sample, y = fct_inorder(species), size = genome_coverage, color = type)) +
  ...
  scale_size_continuous(labels = comma, 
                        ...
                        limits = c(min(subset(data_2$genome_coverage, data_2$genome_coverage != 0)), 
                                    max(data_2$genome_coverage))) +
  labs(x = "", y = "", title = "Genome coverage") +
