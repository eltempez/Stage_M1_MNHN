...
for (file in files) {
  ...
  colnames(file_table) <- c("library", "genome_length", "nb_mapped_reads", "nb_mapped_nt", "p_coverage")
  ...
  data <- data.frame("library" = file_table$library,
                     ...
                     "p_reads" = round((file_table$nb_mapped_reads / nb_aligned_reads * 100), 1),
...
## Dotplot
global_data %>%
  arrange(desc(n_reads)) -> data_2

ggplot(data_2, aes(x = sample, y = fct_inorder(species), size = n_reads, color = type)) +
